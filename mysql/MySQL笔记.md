...
数据管理是指对数据的收集、组织、存储、检索和维护。数据管理技术经历了三个阶段：

### 人工管理阶段（50年代中期以前）

- **背景**：计算机主要用于科学计算，没有磁盘等直接存取的存储设备，没有操作系统和管理数据的软件。
- **特点**：
    - 数据不保存，计算完后数据随程序消失。
    - 没有专用的数据管理软件，数据管理由应用程序自己负责。
    - 数据面向程序，一组数据对应一个程序，数据无法共享。
    - 数据与程序不独立，数据的逻辑结构或物理结构改变，程序必须跟着改。

### 文件系统阶段（50年代后期 ~ 60年代中期）

- **背景**：有了磁盘等存储设备，操作系统中出现了专门的数据管理软件——文件系统。
- **特点**：
    - 数据可以长期保存在磁盘上，反复使用。
    - 由文件系统管理数据，程序与数据有了一定的独立性。
    - 数据面向应用，共享性差，数据冗余大。
    - 数据的逻辑结构与物理结构有简单的对应，但程序员仍需了解物理细节。
- **缺陷**：数据冗余、数据不一致、数据联系弱（文件之间相互独立）。

### 数据库系统阶段（60年代后期至今）

- **背景**：数据量急剧增长，多个用户、多个应用程序需要共享数据，文件系统不再满足需求。
- **特点**：
    - 数据结构化：数据库中的数据不再面向某一个应用，而是面向整个组织，数据之间有联系。
    - 数据的共享性高、冗余度低，易于扩充。
    - 数据独立性高：物理独立性 + 逻辑独立性。
...
| **人工管理**  | **文件系统** | **数据库系统** |         |
| --------- | -------- | --------- | ------- |
| **应用背景**  | 科学计算     | 数据处理      | 大规模数据共享 |
| **数据保存**  | 不保存      | 保存        | 保存      |
| **数据共享**  | 无共享      | 共享性差      | 共享性高    |
| **数据冗余**  | 极高       | 较高        | 低       |
| **数据独立性** | 无        | 较低        | 高       |
| **数据结构化** | 无结构      | 面向文件      | 整体结构化   |
| **管理者**   | 程序员      | 文件系统      | DBMS    |
...
数据库中的数据不再仅仅面向某一个特定应用，而是面向整个组织或企业。数据之间有关联，可以相互引用，整体具有统一的结构。这是数据库系统与文件系统的**根本区别**。

**数据的共享性高、冗余度低**

数据库中的数据可以被多个用户、多个应用程序同时共享使用，相同的数据不需要重复存储多份，从而减少冗余，避免数据不一致。

**数据独立性高**

数据独立性是数据库系统的重要特征，分为两个层次：

- **物理独立性**：数据的物理存储方式（如磁盘格式、文件组织）改变时，应用程序不需要修改。
- **逻辑独立性**：数据库的逻辑结构（如增加新的表或字段）改变时，应用程序不需要修改。

**数据由 DBMS 统一管理和控制**

DBMS 提供以下四方面的数据控制功能：

- **数据的安全性控制（Security）**：防止非法用户访问和破坏数据。
- **数据的完整性控制（Integrity）**：保证数据的正确性和相容性。
- **并发控制（Concurrency Control）**：多用户同时访问数据库时，协调并发操作，避免相互干扰。
- **数据恢复（Recovery）**：系统故障后，将数据库从错误状态恢复到正确状态。
...
- 支持复杂查询、多表关联（JOIN）。
- 支持事务（ACID），保证数据一致性。
- 数据模式固定，修改结构（ALTER TABLE）代价较大。

**代表产品**：MySQL、PostgreSQL、Oracle、SQL Server

### 非关系型数据库（NoSQL，Not Only SQL）

NoSQL 泛指非关系型数据库，并非指"不使用 SQL"，而是对传统关系数据库的补充和扩展。

**主要分类**：

|**类型**|**代表产品**|**数据模型**|**典型场景**|
|---|---|---|---|
|**键值型**|Redis|key → value|缓存、Session 存储|
|**文档型**|MongoDB|JSON 文档|内容管理、用户画像|
|**列族型**|HBase|列族|海量数据存储、日志|
|**图形型**|Neo4j|节点和边|社交网络、知识图谱|

**NoSQL 的主要特点**：

- 数据模型灵活，无需固定的表结构。
- 横向扩展能力强（易于分布式部署）。
- 通常牺牲部分一致性换取高可用和高性能（遵循 BASE 理论）。
- 不支持复杂的关联查询和事务（部分新型 NoSQL 已开始支持）。

**BASE 理论**（NoSQL 的理论基础，对应 ACID）：

- **B**asically Available（基本可用）：系统出现故障时，允许损失部分可用性。
- **S**oft State（软状态）：允许系统存在中间状态，即数据可以有短暂的不一致。
- **E**ventually Consistent（最终一致性）：系统中所有数据副本在经过一段时间同步后，最终都能达到一致状态。

**如何选择**：

|**场景**|**推荐**|
|---|---|
|数据结构固定，需要事务，强一致性要求|关系型数据库（MySQL）|
|数据结构多变，需要高并发读写，海量数据|NoSQL（MongoDB/Redis）|
|需要极速缓存，数据生命周期短|Redis|
...
数据模型是数据库系统中用来**抽象、表示和处理现实世界中数据与信息的工具**，是数据库系统的核心和基础。

### 数据模型的构成要素

数据模型通常由三部分组成，称为**数据模型的三要素**：

**数据结构**

描述数据库中数据的**类型、内容、性质以及数据之间的联系**。它是对系统静态特性的描述，是数据模型最重要的组成部分。通常用数据模型的名称来命名，如关系模型的数据结构是"关系（表）"。

**数据操作**

描述在相应数据结构上可以进行的**操作类型和操作方式**，是对系统动态特性的描述。数据库的操作主要有查询和更新（增、删、改）两大类。

**数据的完整性约束**

数据的完整性约束是一组**完整性规则的集合**，规定数据及其联系所应满足的制约和依存条件，以保证数据的正确性、有效性和相容性。

---

### 概念模型

概念模型（也称**信息模型**）是按用户的观点来对数据和信息建模，独立于具体的 DBMS，主要用于**数据库设计阶段**。

最著名的概念模型是 **E-R 模型**（实体-联系模型，Entity-Relationship Model），由 P.P.Chen 于 1976 年提出。

E-R 模型的基本元素和联系类型详见第6章"数据库设计"部分。

---

### 关系模型

关系模型是目前**最重要、应用最广泛**的数据模型，是关系型数据库（如 MySQL）的理论基础，由 E.F.Codd 于 1970 年提出。

**关系模型的数据结构**：关系（二维表）。

**关系模型的操作**：关系代数（选择、投影、连接、除等），详见第2章"关系代数"部分。

**关系模型的完整性约束**：实体完整性、参照完整性、用户自定义完整性，详见第2章"关系的完整性约束"部分。

**关系模型的优点**：

- 建立在严格的数学概念（集合论、一阶谓词逻辑）基础上。
- 概念单一，无论实体还是联系都用"关系（表）"来表示，数据结构简单清晰。
- 存取路径对用户透明，有更高的独立性，数据保密性更好。

**关系模型的缺点**：

- 查询效率不如非关系型模型（需要 JOIN 等运算）。
- 对于复杂的对象（如图像、音频、层级结构）的表示能力有限。

---

### 数据库领域中的其他数据模型

在关系模型出现之前，还有两种早期的数据模型，目前已较少使用，但在一些遗留系统中仍可见到：

**层次模型（Hierarchical Model）**

用**树形结构**来表示数据及其联系，有且只有一个根节点，其他节点有且只有一个父节点。代表产品：IBM 的 IMS 系统。

- 优点：结构清晰，查询效率高（父子关系明确）。
- 缺点：只能表示 1:N 的联系，无法直接表示 M:N 的联系；数据结构僵硬，插入删除不方便。

**网状模型（Network Model）**

用**有向图结构**来表示数据及其联系，允许一个节点有多个父节点，节点之间可以有多种联系。代表产品：DBTG 系统（CODASYL 规范）。

- 优点：能直接表示复杂的 M:N 联系，性能好。
- 缺点：结构复杂，用户须了解数据的物理存储细节，编程困难，数据独立性差。
...
ANSI/SPARC 数据库系统研究小组在 1975 年提出了数据库系统的**三级模式结构**，这是数据库领域公认的标准框架。

```
用户 A  用户 B  用户 C  用户 D
  |       |       |       |
外模式1  外模式1  外模式2  外模式2   ← 第一级：外模式（视图层）
       ↕             ↕
            模式                    ← 第二级：模式（逻辑层）
             ↕
           内模式                   ← 第三级：内模式（物理层）
...
**外模式（External Schema）—— 用户级**

也称**子模式**或**用户模式**，是数据库用户（包括应用程序员和最终用户）能够看见和使用的**局部数据的逻辑结构和特征的描述**。

- 一个数据库可以有多个外模式，每个用户只能看到和使用对应的外模式。
- 外模式是模式的**子集**，对应 SQL 中的"视图（View）"概念。
- 保证了数据的安全性：用户只能访问外模式中定义的数据，无法接触其他数据。

**模式（Schema）—— 概念级**

也称**概念模式**或**逻辑模式**，是数据库中**全体数据的逻辑结构和特征的描述**，是所有用户的公共数据视图。

- 一个数据库只有**一个**模式。
- 它描述了数据的逻辑结构（有哪些表、每个表有哪些字段、字段的类型）、数据之间的联系，以及数据的完整性和安全性要求。
- 对应 SQL 中的"数据库（Database）"或"Schema"概念。

**内模式（Internal Schema）—— 物理级**

也称**存储模式**，是数据在数据库内部的**物理存储结构和存储方式**的描述。

- 一个数据库只有**一个**内模式。
- 它描述了数据的物理存储结构，如：数据的存储文件格式、记录的存储方式（顺序存储/散列存储）、索引的组织方式等。
- 对应 InnoDB 中的 `.ibd` 文件、B+树索引结构等实现细节。

---

### 数据库系统的二级映像与数据独立性

三级模式之间通过**两个映像**进行转换，这是实现数据独立性的关键。

**外模式 / 模式映像**

定义了某个外模式与模式之间的**对应关系**。当模式改变时（如增加新的表或字段），DBA 只需修改对应的外模式/模式映像，而不需要修改应用程序，从而保证了**逻辑数据独立性**。

> **逻辑数据独立性**：当数据库的逻辑结构（模式）改变时，用户的应用程序不需要改变。这是比物理独立性更难实现的独立性。

**模式 / 内模式映像**

定义了数据全局逻辑结构（模式）与物理存储结构（内模式）之间的**对应关系**。当数据库的存储结构改变时（如换了更高效的存储格式或索引组织方式），DBA 只需修改模式/内模式映像，而不需要修改模式和应用程序，从而保证了**物理数据独立性**。

> **物理数据独立性**：当数据库的内部存储结构改变时，数据库的逻辑结构不变，应用程序也不需要改变。

|**映像**|**连接的两层**|**保证的独立性**|
|---|---|---|
|外模式 / 模式映像|用户级 ↔ 概念级|逻辑数据独立性|
|模式 / 内模式映像|概念级 ↔ 物理级|物理数据独立性|

---

### 数据库系统的组成

一个完整的数据库系统（DBS）由以下部分组成：

**数据库（DB）**：存储在磁盘上的数据集合。

**数据库管理系统（DBMS）**：管理数据库的核心软件，提供数据定义、数据操作、数据控制等功能。主要包括：
...
**应用程序**：基于 DBMS 构建的业务应用系统。

**数据库管理员（DBA，Database Administrator）**：负责全面管理和控制数据库系统的人员，职责包括：

- 决定数据库的信息内容和结构（模式设计）
- 决定数据库的存储结构和存取策略（内模式设计）
- 定义数据的安全性要求和完整性约束
- 监控数据库的使用和运行，负责数据库的性能优化
- 数据库的改进和重组、重构

**用户**：最终使用数据库的人员，通过应用程序或直接通过 DBMS 访问数据。

---

## 数据领域发展趋势

数据库技术自 20 世纪 60 年代诞生以来，经历了三代演变，并在新需求的驱动下持续发展。

### 三代数据库系统

|**第一代**|**第二代**|**第三代**|
|---|---|---|---|
|**时间**|60年代末 ~ 70年代初|70年代初 ~ 80年代初|80年代中期至今|
|**代表**|层次模型、网状模型|关系模型|对象关系模型、扩展关系模型|
|**标志**|有了 DBMS|SQL 标准化|面向对象 + 关系融合|

### 新型数据库技术方向

**分布式数据库（Distributed Database）**

将数据分散存储在网络中的多个节点上，对用户透明地呈现为一个逻辑上统一的数据库。关键技术：数据分片（Sharding）、数据复制、分布式事务。代表：TiDB、CockroachDB。

**大数据技术（Big Data）**

面向海量、高速、多样的数据（即"3V"：Volume/Velocity/Variety），传统关系型数据库难以应对，催生了 Hadoop、Spark 等大数据生态。

**云数据库（Cloud Database）**

将数据库以服务（DBaaS）的形式部署在云端，弹性扩容，按需付费。代表：阿里云 RDS、AWS Aurora、Google Cloud SQL。

**内存数据库（In-Memory Database）**

将数据全部或主要存储在内存中，大幅减少磁盘 IO，适合对延迟极度敏感的场景。代表：Redis、VoltDB。

**NewSQL**

兼顾关系型数据库的 ACID 事务特性和 NoSQL 的横向扩展能力。代表：TiDB、CockroachDB、Google Spanner。

**AI + 数据库**

将机器学习引入数据库内核，实现自动化调优（Auto-tuning）、智能查询优化、异常检测等功能。如 Oracle Autonomous Database、PolarDB 等自治数据库系统。
...
> **安全警告**：极其不建议这样做！因为你的密码会以明文形式留在历史记录里，很容易被别人看到。可以使用mysql -u root -p直接回车，后续输入密码是暗文
...
> 2. **查询字段限制**：分组之后，`SELECT` 后面的字段一般只能是 **分组字段** 和 **聚合函数**。查询其他字段通常没有意义，因为分组之后，一个组里有很多行，或者说数据已经被 “压缩成一行” 了，非分组、非聚合的字段，数据库根本不知道该显示哪一个！
...
在关系型数据库（如 MySQL）的设计中，为了减少数据冗余并保证结构清晰，我们通常会将数据拆分到不同的表里。这些表之间通过主键（Primary Key）和外键（Foreign Key）建立联系。
...
- **概念**：A 表的一行记录可以对应 B 表的多行记录，但 B 表的一行记录只能对应 A 表的一行记录。
    
- **经典案例**：**部门** 与 **员工**。一个部门可以有多个员工，但一个员工只能隶属于一个部门。
    
- **如何实现**：**在“多”的一方（员工表）创建外键**，指向“一”的一方（部门表）的主键。
...
当两张表的关系互相都是“多”的时候，就需要建立多对多关系。

- **概念**：A 表的一行记录可以对应 B 表的多行记录，同时 B 表的一行记录也可以对应 A 表的多行记录。
    
- **经典案例**：**学生** 与 **课程**。一个学生可以选修多门课程，一门课程也可以被多个学生选修。
    
- **如何实现**：**必须建立第三张“中间表”**。中间表至少需要包含两个外键，分别指向两张主表的主键。通常这两个外键也会组合成中间表的联合主键。
    

```
[学生表 (student)]  [学生-课程中间表 (student_course)]  [课程表 (course)]
-----------------  ---------------------------------  ---------------
id (主键) <---------- student_id (外键)                       
                      course_id (外键) -----------------> id (主键)
                                                         name (课程名)
...
一对一关系通常用于表拆分（单表字段太多时进行冷热数据分离，或者为了保护隐私数据）。

- **概念**：A 表的一行记录只能对应 B 表的一行记录，反之亦然。
    
- **经典案例**：**用户基本信息** 与 **用户详情信息**（如身份证号、家庭住址等隐私/低频使用数据）。
    
- **如何实现**：在任意一方加入外键，指向另一方的主键。但必须给这个外键加上 **唯一约束（UNIQUE）**，以确保一条记录只能被绑定一次。
    

```
[用户基本信息表 (user)]            [用户详情表 (user_detail)]
--------------------            -----------------------
id (主键) <-------------------- user_id (外键 + UNIQUE约束)
username (用户名)                id_card (身份证号)
```

### 总结与落地法则

在实际开发中，无论是使用原生的 SQL 还是 Spring Boot 中的持久层框架（如 MyBatis-Plus、JPA），理清表关系都是写出正确 SQL（如 `JOIN` 查询）的前提：

1. **看到“一对多”**：无脑在“多”的一方加外键。
    
2. **看到“多对多”**：必须手动建第三张关联表。
    
3. **多表关联查询**：利用外键作为连接条件（`ON emp.dept_id = dept.id`）进行数据拼接。
...
在关系型数据库中，**多表查询**是指当我们要查询的数据分布在多张不同的表中时，通过连接条件将这些表组合起来，从而一次性检索出我们需要的信息。

多表查询主要分为两大类：**连接查询（Internal/External Join）** 和 **子查询（Subquery）**。

下面我们以员工表（emp）和部门表（dept）为例，带上具体的 SQL 代码来为你系统梳理。

### 数据准备（概念模型）

- **部门表 (dept)**：包含 `id`（主键）, `name`（部门名称）
    
- **员工表 (emp)**：包含 `id`（主键）, `name`（员工姓名）, `age`（年龄）, `dept_id`（外键，指向部门表id）
...
- **概念**：查询两张表**交集**的部分，即只有满足连接条件（外键 = 主键）的记录才会显示。
    
- **语法**：
    
	- **隐式内连接**：使用 `WHERE` 指定连接条件（常见）。
	      `SELECT 字段列表 FROM 表1，表2 WHERE 连接条件...;`
    
    - **显式内连接**：使用 `INNER JOIN ... ON ...`（推荐，语义更清晰）。                  `SELECT 字段 FROM 表1 [INNER] JOIN 表2 ON 连接条件;`
    
    
- **代码例子**：查询所有员工的姓名及其对应的部门名称。
    ...
    _(注：如果某个新员工还没有分配部门，或者某个新部门还没有员工，它们不会出现在内连接的结果中。)_
    

#### 2. 外连接 (LEFT / RIGHT JOIN)

外连接不仅包含符合连接条件的记录，还会包含某一张表的全部记录。

- **左外连接 (LEFT JOIN)**：**包含左表（表1）的所有记录**，以及右表（表2）中符合条件的记录。如果右表没有对应匹配，结果中右表字段显示为 `NULL`。
    
    - **代码例子**：查询**所有员工**的信息及其部门名称（即使这个员工没有部门，也要查出来）。
...
- **右外连接 (RIGHT JOIN)**：**包含右表（表2）的所有记录**，以及左表中符合条件的记录。
    
    - **代码例子**：查询**所有部门**的信息及其对应的员工列表（即使该部门目前没有员工，也要查出来）。
...
- **概念**：同一张表自己连接自己。为了实现自连接，必须给这张表起两个不同的别名。
    
- **常见场景**：表里包含层级关系（如：员工表里有一个 `manager_id` 字段指向该员工直属领导的 `id`）。
    
- **代码例子**：查询员工姓名及其直属领导的姓名。
    
    
    
    ```SQL
    -- 假设 emp 表有 manager_id 字段
...
子查询是指 **SQL 语句中嵌套 SELECT 语句**，也称为嵌套查询。
...
- **概念**：子查询返回的结果是单个值（数字、字符串、日期等）。
    
- **代码例子**：查询研发部（假设研发部部门ID需要去dept表查）的所有员工信息。
...
- **概念**：子查询返回的结果是一列，但有多行数据。常配合 `IN`、`NOT IN`、`ANY`、`ALL` 等关键字使用。
    
- **代码例子**：查询“研发部”和“市场部”所有员工的信息。
...
- **概念**：子查询返回的结果是一行，但包含多个字段。
    
- **代码例子**：查询与“张三”薪资和直属领导**都相同**的其他员工信息。
...
- **概念**：子查询返回的结果是一个虚拟的“表”。通常放在 `FROM` 后面作为临时表，或者配合 `IN` 使用。
    
- **代码例子**：查询入职日期是 “2025-01-01” 之后的所有员工，并显示他们的部门信息。
    
    
    
    ```SQL
    -- 先通过子查询过滤出部分员工，作为一张新表，再与部门表连接
    SELECT e.name, d.name 
    FROM (SELECT * FROM emp WHERE entry_date > '2025-01-01') e
...
1. **确定要查几张表**：分析需求中的字段来源于哪些表（例如：员工姓名在 `emp`，部门名称在 `dept` $\rightarrow$ 需要两表查询）。
    
2. **确定连接条件**：寻找表与表之间的关联字段（通常是 `外键 = 主键`，如 `emp.dept_id = dept.id`）。
    
3. **确定查询类型**：
    
    - 只要两边完全匹配的数据 $\rightarrow$ 用 **`INNER JOIN`**
        
    - 需要保留左表/右表的全部数据 $\rightarrow$ 用 **`LEFT JOIN` / `RIGHT JOIN`**
        
    - 计算步骤复杂，需要先查出一个结果作为条件 $\rightarrow$ 用 **子查询**
...
死锁是指两个或多个事务在并发执行过程中，**各自持有对方所需的锁，同时又在等待对方释放锁**，从而造成永久相互等待、谁都无法继续执行的僵局。
...
**互斥条件**：资源一次只能被一个事务占用，不能共享（排他锁的本质）。

**不剥夺条件**：事务已经获得的资源，在没有主动释放之前，不能被其他事务强行夺走。

**请求并保持条件**：事务在持有已有资源的同时，还在申请新的资源，且申请的资源被其他事务占用时，自己不释放已有资源。

**循环等待条件**：存在一个事务等待链，链中每个事务都在等待下一个事务所持有的资源，形成环路。

### 死锁的预防（破坏必要条件）

**一次封锁法**

要求事务在开始执行前，一次性申请其所需的全部锁。破坏了"请求并保持条件"。

- 优点：简单，绝对不会死锁。
- 缺点：预先难以准确知道需要哪些锁，锁的粒度过大，降低并发度；容易出现"饥饿"（某些事务长期等待资源）。

**顺序封锁法**

预先规定所有资源的加锁顺序，所有事务都按照相同的顺序申请锁。破坏了"循环等待条件"。

- 优点：能防止死锁。
- 缺点：维护资源的加锁顺序本身代价很大；事务的操作顺序往往难以固定，实际应用困难。

### 死锁的检测与解除

预防死锁的代价太大，现代数据库系统（包括 InnoDB）通常**允许死锁发生，然后检测并解除**。

**超时法**

如果一个事务等待锁的时间超过了设定的阈值，则认为发生了死锁，强制回滚该事务。

- 优点：实现简单。
- 缺点：超时时间难以设定；可能误判（某些事务本身就需要较长时间）。

```sql
-- InnoDB 等锁超时时间（秒），默认 50 秒
SHOW VARIABLES LIKE 'innodb_lock_wait_timeout';
SET innodb_lock_wait_timeout = 30;
```

**等待图（Wait-for Graph）检测法**

DBMS 周期性地构造一张**等待图**：节点代表事务，有向边 $T_i → T_j$ 表示事务 $T_i$ 正在等待 $T_j$ 释放锁。如果等待图中出现**环路**，则说明发生了死锁。

InnoDB 默认开启死锁检测（`innodb_deadlock_detect = ON`），一旦检测到死锁，会**选择一个代价最小的事务（通常是回滚代价最小的）作为牺牲者（Victim）进行回滚**，从而打破环路，让其他事务继续执行。
...
1. **固定加锁顺序**：多个事务访问多张表时，始终按照相同的顺序加锁（如：始终先锁小 ID 的行）。
2. **缩短事务**：尽量将事务设计得短小，减少持锁时间和持锁范围。
3. **降低隔离级别**：在业务允许的情况下，使用 READ COMMITTED 代替 REPEATABLE READ，减少间隙锁的使用。
4. **使用索引**：确保 UPDATE/DELETE 语句能命中索引，避免行锁升级为表锁，降低锁冲突概率。
5. **一次性加锁**：同一事务中，尽量将多个 `SELECT ... FOR UPDATE` 合并为一次操作。
...
活锁是指事务并没有被"冻结"（不像死锁那样完全阻塞），而是一直处于**活跃的重试状态**，但始终无法取得进展的情况。事务在不断地"礼让"或"重试"，却因为时序总是凑巧，永远轮不到自己成功执行。

> **类比**：两个人在走廊里迎面相遇，都往同一方向让路，反复碰撞，谁都过不去——但两个人都在动，不是"卡死"，这就是活锁。

### 活锁的典型场景

在数据库中，活锁常发生于**优先级调度不当**或**抢占式策略**下：

- 事务 A 申请某资源，被事务 B 和 C 先后抢占。
- 事务 A 不断地撤退（退让）并重试，但每次重试时都有新的事务插队，导致 A 永远得不到执行机会。

```
时刻1：事务A 申请锁R，事务B 先获得
时刻2：事务B 释放，事务A 再次申请，事务C 先获得
时刻3：事务C 释放，事务A 再次申请，事务D 先获得
...
事务 A 无限重试，永远得不到锁，但系统中其他事务在正常运行
```
...
| ---------------- | ---------------- | ------------- |
| **事务状态**         | 完全阻塞，不再执行        | 持续活跃，不断重试     |
| **资源占用**         | 事务持有资源并等待        | 事务反复申请但得不到    |
| **系统表现**         | 进程卡死，需要外部干预      | 系统忙碌，但特定事务无进展 |
| **直觉类比**         | 两车顶牛，谁都不动        | 两人让路，反复碰撞     |
| **解决方向**         | 检测到死锁后回滚牺牲者      | 引入公平调度机制      |

### 活锁的解决——先来先服务（FCFS）

解决活锁最有效的方法是引入**公平队列**机制，即**先来先服务（FCFS，First Come First Served）**：

- 当事务申请的锁被其他事务占用时，将该申请加入**等待队列**，按照申请的先后顺序排队。
- 当资源被释放时，优先将资源分配给**等待时间最长**的事务，而不是随机选择或让新来的事务插队。

这样就能保证每个事务的请求**最终一定会得到响应**，从根本上消除活锁的可能性。

```
等待队列（FCFS）：
[事务A（等待最久）] → [事务B] → [事务C（刚加入）]

资源释放时：优先给事务A，而非随机
```

> InnoDB 内部通过锁的等待队列机制来保证一定程度上的公平性，在正常情况下可以有效避免活锁。
