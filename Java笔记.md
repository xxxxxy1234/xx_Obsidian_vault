...
- **直接赋值**：`String s1 = "abc";`
    ...
    JVM 会先去常量池找有没有 "abc"。有，就直接复用地址；没有，就创建一个放进去。
    
- **new 构造器**：`String s2 = new String("abc");`
    
    每执行一次 `new`，都会在堆内存中开辟一个**全新**的空间。
...
System.out.println(s1 == s2);      // true (都在常量池，同一个地址)
System.out.println(s1 == s3);      // false (s3在堆内存，地址不同)
