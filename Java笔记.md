...
这些是最基础的匹配规则，用于匹配**单个字符**。（若想匹配两个字符用两次规则即可 ，如【abc】【abc】）
...
| **`[a-z&&[def]]`** | 交集：d、e 或 f              | 必须同时满足两个条件                   |

---

### 2. 预定义字符（元字符）

在 Java 中使用这些符号时，通常需要加双反斜杠进行转义（如 `\\d`）。
...
**温馨提示**：正则表达式虽然强大，但过于复杂的正则会降低代码可读性。建议配合注释使用，或将其定义为常量。
