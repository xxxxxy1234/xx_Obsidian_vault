...
所有的字符输入流类（如 `FileReader`）都继承自 `Reader` 抽象类，其中 `read()` 方法是读取数据的核心。

---

### 字符输入流的 `read` 方法详解

`read()` 方法主要有三种重载形式，根据不同的需求，你可以选择一次读取一个字符、读取一组字符，或将字符读入特定的缓冲区。
...
- **返回值**：
    ...
    - 返回的是字符对应的 **编码十进制整数**（例如字母 'a' 返回 97，汉字 '你' 返回其 Unicode 码点）。
        
    - 如果读到文件末尾，则返回 **`-1`**。
        
- **特点**：每次调用只会从硬盘读取一个字符，由于频繁操作硬盘，效率较低。
...
int ch;
while ((ch = fr.read()) != -1) {
    System.out.print((char) ch); // 将读取到的数字强转为字符显示
}
fr.close();
```
...
#### 2. `int read(char[] cbuf)`：一次读取一个字符数组

这是**效率最高**且最常用的方式。

- **功能**：尝试从输入流中读取字符，并存入提供的字符数组 `cbuf` 中。
...
- **特点**：通过缓冲区（数组）减少了读取硬盘的次数，大大提升了 IO 性能。
...
#### 3. `int read(char[] cbuf, int off, int len)`：读取部分数据到数组

这是数组读取方式的精细版，提供了更强的灵活性。

- **参数说明**：
    
    - `cbuf`：存放字符的数组。
        
    - `off`：在数组中开始存放字符的偏移量（索引位置）。
        
    - `len`：最多读取的字符个数。
        
- **功能**：从流中读取最多 `len` 个字符，从数组的 `off` 位置开始存放。
    
- **返回值**：实际读取的字符数，或到达末尾时返回 `-1`。
...
1. **自动解码**：字符流会根据系统默认编码（如 UTF-8）读取字节，并在内存中将其转化为字符。如果你读取的文件编码与系统默认不一致，即便使用字符流也可能出现乱码（此时需要使用 `InputStreamReader` 指定字符集）。
    
2. **资源释放**：字符输入流使用完毕后，必须调用 `close()` 方法释放系统资源。在实际开发中，推荐使用 JDK 7 引入的 **try-with-resources** 语法。
    
3. **缓冲区大小**：在使用 `read(char[] cbuf)` 时，字符数组的大小建议设置为 **1024 的整数倍**（如 8192，即 8KB），这是大多数操作系统的最佳缓冲大小。
    

**总结建议**：除非是处理非文本文件（如图片、音频），否则读写文本请务必使用字符流（Reader/Writer），因为它们内部处理了字符集映射，让你的代码更稳健。
