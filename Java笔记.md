...
- **作用**：它会把 `ContentPane`（内容面板）这个大容器里所有的子组件全部“踢出去”。
    
- **类比**：就像重新画一幅画之前，必须先把旧的画布撕掉，换一张白纸。
    

#### 2.3 repaint()：强制重绘

- **作用**：告诉 JVM 的 GUI 绘制线程（EDT）：“喂，界面数据变了，赶紧重新画一遍给我看！”
    
- **为什么必须手动调？**：为了节省性能，Swing 不会时刻盯着你的代码看。你删了组件、加了新组件，在内存里数据变了，但显示器上的像素点还没变。调用 `repaint()` 才能同步内存和屏幕。
