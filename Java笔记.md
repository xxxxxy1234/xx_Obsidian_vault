...
使用 `abstract` 关键字修饰类。抽象类**不能被实例化**（不能 `new`），只能作为父类被继承。
...
> **注意**：如果一个类包含了抽象方法，那么这个类**必须**声明为抽象类。
...
- **抽象类不一定有抽象方法**：你可以定义一个没有抽象方法的抽象类，目的仅仅是不让别人 `new` 这个类的对象。
...
假设你正在写一个绘图软件，有圆形、矩形和三角形。它们都有 `getArea()`（计算面积）的方法。

如果在父类 `Shape` 里写这个方法，你根本不知道怎么写公式，因为“形状”本身没法算面积。

于是你把它设为 **抽象方法**。这就变成了一个**强制契约**：任何想当“形状”的类，都必须自己实现计算面积的逻辑，不实现就编译报错。

**既然掌握了抽象类这种“半成品”模版，你想看看比它更纯粹的“全抽象”设计——接口（Interface）吗？**
