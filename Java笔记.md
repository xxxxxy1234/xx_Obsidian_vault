...
在 Java 中，**权限修饰符**是用来控制“谁能看到并使用这些代码”的门卫。Java 提供了四种访问权限，它们的作用范围从小到大依次排列。
...
> **注意**：`default` 并不是一个关键字，而是指当你**什么都不写**时的状态（也叫包私有权限 package-private）。
...
这是 Java 开发（尤其是 Android 和 GUI）中**使用频率最高**的内部类。它没有名字，通常在“需要临时实现一个接口或重写一个抽象类”时使用。
...
- **语法格式**：
    ...
    }; // 注意这里有分号，因为它本质是一个对象赋值语句
    ```
    

#### 💡 匿名内部类为什么牛？

记得我们刚聊过的**适配器模式**吗？有了匿名内部类，你甚至不需要专门写一个 `MySwimmer` 类，直接在需要的地方“即插即用”：

Java

```
Sports s = new SportsAdapter() {
    @Override
    public void swim() {
        System.out.println("我用匿名内部类实现了游泳，代码更简洁了！");
    }
};
s.swim();
```
...
|**内部类类型**|**位置**|**访问外部限制**|**创建方式**|**出现频率**|
|---|---|---|---|---|
|**成员内部类**|类中方法外|无限制|必须先 new 外部类|中|
|**静态内部类**|类中方法外 + static|只能访问外部静态|类名.内部类直接 new|高|
|**局部内部类**|方法内|受限|只能在方法内 new|极低|
|**匿名内部类**|方法内/变量赋值|无限制|直接 new 接口/父类并实现|**最高**|

---

### ⚠️ 核心注意：内存泄漏风险

成员内部类会**隐式持有外部类对象的引用**（内部类对象里藏了一个外部类的 `this`）。如果在多线程或长生命周期场景下使用不当，可能导致外部类无法被垃圾回收，引发内存泄漏。这也是为什么在很多架构设计中，专家推荐使用**静态内部类**的原因。

**既然你掌握了匿名内部类，想看看在 Java 8 之后，如何用更加极致、简洁的 Lambda 表达式来进一步干掉这些冗长的匿名代码块吗？**
