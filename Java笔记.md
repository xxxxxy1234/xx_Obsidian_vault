...
### 排序方式一：自然排序 (Natural Ordering)

这是 **“让元素实现自律”** 的方式。要求存入的类必须实现 `Comparable` 接口，并重写 `compareTo` 方法。

- **原理**：类自己定义比大小的逻辑。
    
- **适用场景**：类自带明显的先后属性（如数字大小、字母顺序）。
...
### 排序方式二：比较器排序 (Comparator Ordering)

这是 **“外部强制干预”** 的方式。在创建 `TreeSet` 对象时，传入一个 `Comparator` 比较器。

- **原理**：集合本身自带一套裁判标准。
    
- **适用场景**：
    
    1. 存入的类（如 `String`）已经写好了排序规则，但不是你想要的（比如你想按长度排）。
        
    2. 存入的类是你引用的第三方类，你没法改它的源码去实现接口。
...
- 如果你想让集合**自动按从小到大排队**：选 `TreeSet`。
    
- 如果对象本身有比较属性：实现 `Comparable`。
    
- 如果想自定义特殊的排序逻辑：构造时传入 `Comparator`。
