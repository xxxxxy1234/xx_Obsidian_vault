...
按照你提供的图片风格，我为你整理了 `Collection` 的核心 API 方法表。
...
2. **动作**：它每取出一个元素，就会把这个元素交给你的 **Lambda 表达式**（即 `Consumer` 接口的 `accept` 方法）去处理。
    

底层逻辑示意代码：
```java
// 假设这是 ArrayList 内部的实现
public void forEach(Consumer<? super E> action) {
    for (E e : this) { // 还是在用增强 for 或 数组遍历
        action.accept(e); // 把元素传给你的 Lambda 代码块
    }
}
```
