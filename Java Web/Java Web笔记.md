...
既然项目已经在 IDEA 里妥善安家了，接下来：

- **你是想看看 Maven 的“依赖传递性”（即 A 包自动带出 B 包，产生冲突时该怎么用 `<exclusion>` 标签进行精准排除）？**
    
- **还是想去研究一下 Maven 极为高频的“生命周期与构建命令”（比如 `clean` 到底清理了什么，`package` 又是怎么把项目打包的）？**