...
在 MySQL 中，视图不仅可以用来简化查询，还可以进行 `INSERT` 或 `UPDATE` 操作。为了保证通过视图修改的数据依然符合视图定义的筛选条件，我们需要使用 `WITH CHECK OPTION` 子句。

这个选项有两个关键字：**CASCADED（级联）** 和 **LOCAL（本地）**。它们决定了在视图嵌套（即视图基于另一个视图创建）的情况下，检查规则如何传递。

---

### 1. CASCADED (级联检查) —— 默认选项

这是最严格、最常用的模式。如果一个视图定义了 `WITH CASCADED CHECK OPTION`，那么在更新数据时，MySQL 不仅会检查**当前视图**的条件，还会**递归地检查它依赖的所有底层视图**的条件，无论底层视图是否定义了检查选项。

#### 例子演示：

1. **视图 v1**：`SELECT * FROM table WHERE id <= 20;`（未定义检查选项）
    
2. **视图 v2**：基于 v1，`SELECT * FROM v1 WHERE id >= 10 WITH CASCADED CHECK OPTION;`
...
- 当你通过 **v2** 插入数据时：
    ...
    - 由于是 **CASCADED**，它会向上追溯，检查 v1 的条件：`id <= 20`。
        
- **结论**：只有 `10 <= id <= 20` 的数据能插入成功。
...
`LOCAL` 检查选项相对更加“自私”一些。它在更新数据时，会检查**当前视图**的条件。对于其依赖的底层视图，它会去查看底层视图**是否也显式定义了检查选项**。如果底层定义了，就检查；如果底层没定义，就不检查。

#### 例子演示：

1. **视图 v1**：`SELECT * FROM table WHERE id <= 20;`（未定义检查选项）
    
2. **视图 v2**：基于 v1，`SELECT * FROM v1 WHERE id >= 10 WITH LOCAL CHECK OPTION;`
...
- 当你通过 **v2** 插入数据时：
    ...
    - 查看 v1 是否有检查选项？**没有**。于是跳过 v1 的检查。
        
- **结论**：只要 `id >= 10` 的数据都能插入成功（即使 `id = 100` 也可以，尽管这样数据会在 v1 和 v2 中“消失”，因为不符合查询条件）。
...
|---|---|---|
|**检查当前视图条件**|是|是|
|**检查底层视图条件**|**强制检查**所有底层视图条件|**只检查**定义了检查选项的底层视图|
|**默认行为**|是 MySQL 的默认值|需要显式声明|
|**安全性**|极高，防止数据偏移|中等，给底层留有余地|

---

### 为什么需要这两个选项？

如果没有这些检查选项，你可能会遇到以下尴尬场景：

1. 你建立了一个视图 `v_students_pass` 专门显示及格的学生（`score >= 60`）。
    
2. 某人通过视图把一个学生的成绩 `UPDATE` 成了 50 分。
    
3. 更新成功了，但是这个学生在视图里**突然消失了**（因为不符合视图的 `WHERE` 条件了）。
    

**总结建议：**

- 在大多数生产环境下，建议使用 **`CASCADED`**，因为它能确保数据的一致性，防止数据通过视图修改后变成“幽灵数据”（基表有，视图查不到）。
    
- 只有当你明确知道底层视图的限制可以被“临时忽略”时，才考虑使用 `LOCAL`。
...
1. **性能陷阱**：视图并不加速查询！相反，如果你在视图上再套视图，或者视图里包含复杂的聚合函数，性能可能会非常糟糕。因为它本质上还是在执行那段长 SQL。
    
2. **不可更新的情况**：如果视图包含以下内容，则不允许 `UPDATE` 或 `INSERT`：
