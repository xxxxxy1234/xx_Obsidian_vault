...
- **显式内连接**：使用 `INNER JOIN ... ON ...`（推荐，语义更清晰）。 SELECT 字段列表 FROM 表1 [INNER] JOIN 表2 ON 连接条件...;
...
- **右外连接 (Right Join)**：返回右表所有记录，左表不匹配的显示为 `NULL`。ELECT 字段列表 FROM 表1 RIGHT [OUTER] JOIN 表2 ON 条件...;
