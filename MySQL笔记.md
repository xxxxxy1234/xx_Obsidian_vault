...
4. **eq_ref**：唯一性索引扫描，多表连接时常见。
    
5. **ref**：非唯一性索引扫描（返回匹配某个值的多行）。
    
6. **range**：索引范围扫描（如 `between`、`>` 、`<`）。
    
7. **index**：全索引扫描（把索引树翻一遍，通常比 ALL 快一点点）。
...
- **Using index**：出现了**覆盖索引**，性能极好（不需要回表）。
...
## 是否走索引看的是select还是where


这是一个非常深刻的问题。答案是：**主要看 `WHERE`，但 `SELECT` 决定了优化的上限。**

更准确地说：**`WHERE` 决定能不能走索引，`SELECT` 决定走索引的效率（是否需要回表）。**

---

### 1. WHERE：索引的“入场券”

`WHERE` 条件是决定 MySQL **是否使用索引**进行数据定位的关键。

- **决定性**：如果 `WHERE` 子句中的字段没有索引，或者违背了“最左前缀法则”、“索引列运算”等规则，MySQL 根本无法在索引树上进行二分查找。
    
- **动作**：`WHERE` 告诉引擎：“请帮我在这几百万行里，把符合条件的这几行**找出来**。”
...
`SELECT` 决定了 MySQL **拿到索引后该怎么做**。

- **影响效率**：如果你 `SELECT` 的字段都在索引树里，就触发了**覆盖索引**，不需要回表，速度飞快。
    
- **拖后腿**：如果你 `SELECT *` 或者选了索引之外的字段，即使 `WHERE` 走了索引，MySQL 还要拿着 ID 回到主键索引树去查整行数据（**回表**）。
    
- **极端情况**：有时候 `WHERE` 没走索引，但因为 `SELECT` 的字段都在索引里，MySQL 会为了省事直接扫描整个索引树（`type=index`），这虽然也叫“走索引”，但性能不如精准定位。
...
假设联合索引为：`idx_user(name, age)`

|**场景**|**查询语句**|**谁在起作用**|**最终表现**|
|---|---|---|---|
|**标准走索引**|`WHERE name='A' SELECT *`|`WHERE` 引导|走索引，但需要回表（中等）|
|**覆盖索引**|`WHERE name='A' SELECT age`|`WHERE` + `SELECT`|**性能巅峰**，不回表（极快）|
|**强制索引扫描**|`SELECT name, age` (无WHERE)|`SELECT` 引导|扫描整个索引树，不扫全表（快）|
|**索引失效**|`WHERE age=20 SELECT *`|`WHERE` 搞砸了|全表扫描（最慢）|

---

### 4. 总结

- **`WHERE` 是为了“过滤”**：它通过索引树快速缩小范围。如果它失败了，大概率就是全表扫描（`ALL`）。
    
- **`SELECT` 是为了“取值”**：它决定了是否需要二次查询（回表）。
    

**设计建议：**

先根据 `WHERE` 频繁出现的字段建立索引（保证能入场），再根据 `SELECT` 常用的字段建立联合索引（实现覆盖索引，跑得更快）。
