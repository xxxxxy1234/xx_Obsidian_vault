...
虽然全局锁能保证一致性，但它的副作用非常明显：

- **业务停摆**：如果在主库上加锁，备份期间整个业务无法写入数据，导致 App 或网站无法正常运行。
    
- **延迟堆积**：如果在从库上加锁，备份期间从库无法执行主库同步过来的 binlog，导致主从延迟激增。
    

---

### 5. 更好的替代方案：--single-transaction

如果你使用的存储引擎是 **InnoDB**（支持事务），其实不需要使用影响如此之大的全局锁。

在备份时使用 `mysqldump` 的 `--single-transaction` 参数：

- **原理**：它利用了 InnoDB 的 **MVCC（多版本并发控制）** 技术。在备份开始时开启一个一致性视图（Snapshot）。
    
- **优点**：备份过程中，其他事务依然可以正常读写数据，既保证了备份数据的一致性，又不影响业务运行。
