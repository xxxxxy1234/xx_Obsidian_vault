...
>
>互斥规则如下（主要针对与表锁的冲突）：- **意向锁之间兼容**：多个事务可以同时持有同一张表的 IS 或 IX 锁。意向锁之间不会互相阻塞，因为它们只是为了表达“我正锁着表里的某几行”。
...
**聊完了全局锁、表级锁（表锁、MDL、意向锁），现在终于来到了大家最关心、也是面试最高频的部分——行级锁。你想了解行锁是怎么解决并发冲突，又是怎么产生“间隙”的吗？**