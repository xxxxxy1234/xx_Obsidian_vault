...
它的主要作用是**维护表元数据的一致性**，防止在执行查询或事务的过程中，表结构（Metadata）被其他线程修改，从而导致数据不一致。
...
|**锁类型**|**对应操作**|**兼容性**|
|---|---|---|
|**MDL 读锁**|`SELECT`, `UPDATE` 等数据操作|**兼容**。多个线程可以同时操作数据。|
|**MDL 写锁**|`ALTER`, `DROP` 等表结构修改|**互斥**。在修改表结构时，无法进行数据读写；反之亦然。|

### 3. 典型冲突场景

如果你在一个长事务中执行了 `SELECT`（持有 MDL 读锁），此时另一个线程尝试执行 `ALTER TABLE`（申请 MDL 写锁），该 DDL 操作会被阻塞，直到长事务提交或回滚释放读锁。
